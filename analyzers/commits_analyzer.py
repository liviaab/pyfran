...
class CommitsAnalyzer:
    ...
    def process_metrics(self):
        first_hash_unittest = None or \
            (self.unittest_occurrences.has_first_occurrence() and self.unittest_occurrences.first['commit_hash'])
        last_hash_unittest = None or \
            (self.unittest_occurrences.has_last_occurrence() and self.unittest_occurrences.last['commit_hash'])
        first_hash_pytest = None or \
            (self.pytest_occurrences.has_first_occurrence() and self.pytest_occurrences.first['commit_hash'])

        if not first_hash_unittest or not last_hash_unittest or not first_hash_pytest:
    ...
    def __update_occurrences(self, commit, modification):
        ...
        if self.memo["unittest_in_removed_diffs"] and (not self.memo["unittest_in_code"]):
        ...
        if self.memo["pytest_in_removed_diffs"] and not self.memo["pytest_in_code"] \
